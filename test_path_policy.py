...
class RuntimePathPolicyTests(unittest.TestCase):
    ...
    def test_worker_output_requires_fixed_family_and_validated_evidence_id(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(written.relative_to(root).as_posix(), "reports/evidence-001.json")
            for unsafe in ("reports/../escape.json", "other/evidence-001.json", "reports/bad id.json"):
