...
def _evaluate_gates(metrics: dict[str, Any], profile: LoadSoakProfile) -> dict[str, bool]:
    ...
    return {
        ...
        "replay_did_not_duplicate_findings": metrics["persisted_findings"] == profile.unique_findings,
...
def _validate_metrics(metrics: Any, profile: LoadSoakProfile) -> None:
    expected = {
        ...
        "persisted_findings", "delivered_events", "errors", "error_classes",
        "elapsed_seconds", "throughput_per_second", "latency_ms", "cpu_seconds",
    ...
    integer_fields = (
        "operations", "unique_findings", "expected_reassessments",
        "persisted_findings", "delivered_events", "errors", "peak_traced_bytes",
    )
    if any(isinstance(metrics[name], bool) or not isinstance(metrics[name], int) or metrics[name] < 0 for name in integer_fields):
    ...
    if (
        ...
        or metrics["expected_reassessments"] != profile.unique_findings * profile.replay_rounds
    ):
    ...
    outbox = metrics["outbox"]
    if not isinstance(outbox, dict) or set(outbox) != {
        "delivered", "pending", "dead", "retrying", "oldest_pending_age_seconds"
    }:
