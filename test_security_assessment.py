...
class SecurityAssessmentTests(unittest.TestCase):
    ...
    def test_current_repository_passes_offline_without_live_credit(self):
        ...
        self.assertEqual(document["assessment_decision"], "PASS_OFFLINE" if ASSESSMENT_YAML_AVAILABLE else "NO_GO")
        self.assertEqual(not document["findings"], ASSESSMENT_YAML_AVAILABLE)
        self.assertEqual(
    ...
    @unittest.skipUnless(ASSESSMENT_YAML_AVAILABLE, "requires hash-locked PyYAML")
    ...
    def test_container_policy_uses_effective_user_and_copy_directives(self):
        ...
        self.assertTrue(_container_file_passes(valid))
        self.assertFalse(
            _container_file_passes(misleading_user)
        )
        self.assertFalse(
            _container_file_passes(broad_copy)
        )
        for source in ("app/..", "./subdir/../", "../app", "/app", "C:/app", "${SRC}", "$FILE"):
            with self.subTest(source=source):
                ...
                self.assertFalse(
                    _container_file_passes(dockerfile)
                )
        ...
        self.assertFalse(
            _container_file_passes(malformed_base)
        )

    def test_container_policy_rejects_malformed_sha256_references(self):
        complete_digest = "a" * 64
        malformed_digests = (
            "a" * 63,
            "a" * 65,
            "g" * 64,
            complete_digest + ":latest",
        )
        for digest in malformed_digests:
            with self.subTest(digest=digest):
                external_from = (
                    f"FROM registry/image@sha256:{digest}\n"
                    "COPY app.py /app/app.py\nUSER 10001:10001\n"
                )
                external_copy = (
                    f"FROM python@sha256:{complete_digest}\n"
                    f"COPY --from=registry/image@sha256:{digest} /src /app/src\n"
                    "USER 10001:10001\n"
                )
                self.assertFalse(
                    _container_file_passes(external_from)
                )
                self.assertFalse(
                    _container_file_passes(external_copy)
                )
    ...
    def test_python_only_control_separators_are_not_buildkit_whitespace(self):
        digest = "a" * 64
        base = f"FROM python@sha256:{digest}\nUSER 10001:10001\n"
        for separator in ("\x1c", "\x1d", "\x1e", "\x1f"):
            with self.subTest(position="before_hash", separator=ord(separator)):
                self.assertIsNone(
                    _docker_instructions(f"{separator}# escape=`\n" + base)
                )
            with self.subTest(position="after_hash", separator=ord(separator)):
                parsed = _docker_instructions(f"#{separator}escape=`\n" + base)
                self.assertIsNotNone(parsed)
                self.assertEqual(parsed.escape_character, chr(92))

    def test_control_separators_do_not_prefix_docker_instructions(self):
        for separator in ("\x1c", "\x1d", "\x1e", "\x1f"):
    ...
    def test_docker_instruction_arguments_preserve_control_separators(self):
        separator = "\x1f"
        parsed = _docker_instructions(
    ...
    def test_docker_continuations_require_odd_trailing_escape_count(self):
        ...
        self.assertEqual(odd[0], "RUN echo ")
    ...
    def test_container_policy_rejects_all_add_instructions(self):
        ...
        for source in (
            ...
            ".",
            "archive.tar.gz",
            "https://example.invalid/untrusted.tar.gz",
        ):
            with self.subTest(source=source):
                dockerfile = (
                    f"FROM python@sha256:{digest}\n"
    ...
    def test_container_policy_rejects_build_context_copy_globs(self):
        digest = "a" * 64
        for source in ("*", "app/*", "config?.json", "app/[ab].py"):
            with self.subTest(source=source):
                dockerfile = (
                    f"FROM python@sha256:{digest}\n"
                    f"COPY {source} /app/\n"
    ...
    def test_container_policy_validates_flags_stages_and_external_images(self):
        ...
        unicode_numeric_stage = (
            f"FROM {pinned}\nCOPY --from={arabic_indic_one} /src /app/src\n"
            "USER 10001:10001\n"
        )
        for dockerfile in (
        ...
        for dockerfile in (
            mutable_external_copy, mutable_external_from, unicode_numeric_stage
        ):
            with self.subTest(dockerfile=dockerfile):
                self.assertFalse(
                    _container_file_passes(dockerfile)
                )
        allowed = frozenset({"${RUNTIME_IMAGE}"})
        overlay = "FROM ${RUNTIME_IMAGE}\nUSER 10001:10001\n"
        mutable_overlay = overlay + "FROM registry/image:latest\nUSER 10001:10001\n"
        self.assertTrue(_container_file_passes(overlay, allowed))
        self.assertFalse(_container_file_passes(mutable_overlay, allowed))
    ...
    def test_cli_writes_only_fixed_sanitized_output(self):
        ...
        self.assertEqual(result, 0 if ASSESSMENT_YAML_AVAILABLE else 1)
        ...
        error_stdout, error_stderr = StringIO(), StringIO()
        with redirect_stdout(error_stdout), redirect_stderr(error_stderr):
            error_result = main([
                "--source-commit", "invalid",
                "--dependency-scan-outcome", "success",
                "--ci-run-id", "12345",
            ])
        self.assertEqual((error_result, error_stdout.getvalue()), (2, ""))
