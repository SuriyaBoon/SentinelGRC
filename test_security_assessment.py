...
class SecurityAssessmentTests(unittest.TestCase):
    ...
    def test_homoglyph_user_cannot_satisfy_non_root_container_control(self):
        digest = "a" * 64
        dockerfile = (
            f"FROM python@sha256:{digest}\n"
            "u\u017fer 10001:10001\n"
        )
        self.assertIsNone(_docker_instructions(dockerfile))
        self.assertFalse(_container_file_passes(dockerfile))

    def test_single_quoted_backslash_matches_pinned_builder_flag_scanner(self):
        flags, remaining = _extract_builder_flags(
            r"--chown='foo\bar' app.py /app/", "\\"
        )
        self.assertEqual(flags, ("--chown=foobar",))
