...
class SupplyChainPolicyTests(unittest.TestCase):
    ...
    @unittest.skipUnless(yaml is not None, "requires hash-locked PyYAML")
    ...
    def test_publication_requires_unprivileged_qualification_and_exact_handoff(self):
        ...
        for gate in (
            ...
            'sentinelgrc:release -m unittest -v "${runtime_test_modules[@]}"',
            "sentinelgrc-qualification:release -m unittest -v",
        ...
        partition = re.search(
            r"runtime_test_modules=\(\s*(.*?)\s*\)", qualify, re.DOTALL
        )
        ...
        expected_runtime_modules = (
            ...
            "test_postgres_integration test_postgres_runtime_state test_pre_live_reliability test_production_contract test_portfolio_contracts test_reporting test_runtime_app test_security_alert_contract test_security_event_connector "
        ...
        self.assertIn("--platform linux/amd64", qualify)
        qualification = re.search(r"sentinelgrc-qualification:release -m unittest -v \\\s*([^\n]+)", qualify)
