...
class ProductionImageClosureTests(unittest.TestCase):
    ...
    def test_qualification_overlay_adds_only_hash_locked_assessment_dependencies(self) -> None:
        ...
        self.assertIn(("ARG", "RUNTIME_IMAGE=sentinelgrc:runtime-image-required"), instructions)
        self.assertIn(("FROM", "${RUNTIME_IMAGE}"), instructions)
        copies = [argument for directive, argument in instructions if directive == "COPY"]
        self.assertEqual(
            copies,
            [
        ...
        runs = [argument for directive, argument in instructions if directive == "RUN"]
        self.assertEqual(len(runs), 1)
        self.assertIn(
            "--require-hashes --requirement /tmp/requirements-assessment-hashed.txt",
            runs[0],
        )
        users = [argument for directive, argument in instructions if directive == "USER"]
