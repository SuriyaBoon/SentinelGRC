...
Governed task: DEV-ddd863e5683f35e8
...
ASCII_AUDIT_FILES = (
    REPO_ROOT / "docs" / "hermetic-ci-runtime-audit.md",
    REPO_ROOT / "docs" / "azure-live-validation.md",
    MANIFEST_PATH,
    WHITELIST_PATH,
    Path(__file__),
)
...
class ProductionImageClosureTests(unittest.TestCase):
    ...
    def test_docker_python_copy_set_matches_positive_manifest(self) -> None:
        self.assertEqual(_read_manifest(MANIFEST_PATH), self.copied)
    ...
    def test_manifest_is_sorted_and_has_no_duplicates(self) -> None:
        ...
        self.assertEqual(sorted(set(entries)), entries)
    ...
    def test_type_checking_imports_do_not_create_runtime_dependencies(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(set(), _imports_for_file(root, "app.py").local_imports)
    ...
    def test_importlib_calls_are_detected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("plugin", calls[0]["target"])
