...
class OfflineEvidenceTests(unittest.TestCase):
    ...
    def test_validation_order_is_stable_across_extracted_helpers(self):
        baseline = collect_offline_evidence(POLICY, ALERTS, SOURCE_COMMIT)["document"]
        cases = []

        identity_first = copy.deepcopy(baseline)
        identity_first["mode"] = "invalid"
        identity_first["inputs"]["source_contract"] = "invalid"
        cases.append((identity_first, "offline evidence identity is invalid"))

        inputs_before_results = copy.deepcopy(baseline)
        inputs_before_results["inputs"]["source_contract"] = "invalid"
        inputs_before_results["results"]["alert_count"] = 0
        cases.append((inputs_before_results, "offline evidence source contract is invalid"))

        results_before_boundary = copy.deepcopy(baseline)
        results_before_boundary["results"]["alert_count"] = 0
        results_before_boundary["claim_boundary"]["current_live_gate_credit"] = True
        cases.append((results_before_boundary, "offline evidence alert count is invalid"))

        for document, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(ValueError, message):
                    create_evidence_envelope(document)
