...
def _from_reference(
    ...
    immutable = internal or not require_pinned_base or _is_digest_pinned_reference(image)
...
def _containers_are_pinned_non_root(root: Path) -> tuple[bool, str]:
    ...
    overlays_passed = all(
        _container_file_passes(content, require_pinned_base=False)
        for content in (assurance, qualification)
    )
    return runtime_passed and overlays_passed, (
