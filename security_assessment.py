...
def _docker_escape_character(
    line: str,
    current: str,
    instruction_seen: bool,
    directive_seen: bool,
) -> tuple[str, bool] | None:
    """Validate an optional top-level Dockerfile escape parser directive."""
    matched = DOCKER_ESCAPE_DIRECTIVE.fullmatch(line)
    if matched is not None:
        if instruction_seen or directive_seen:
            return None
        return matched.group(1), True
    return None if re.match(r"^#\s*escape\b", line, re.IGNORECASE) else (current, directive_seen)


def _docker_instructions(text: str) -> DockerfileSpec | None:
    """Parse directives and retain the reviewed Dockerfile escape character."""
    ...
    for raw_line in text.splitlines():
        ...
        if not line:
            continue
        if line.startswith("#"):
            parsed_escape = _docker_escape_character(
                line, escape_character, instruction_seen, escape_directive_seen
            )
            if parsed_escape is None:
                return None
            escape_character, escape_directive_seen = parsed_escape
            continue
        instruction_seen = True
        continued = line.endswith(escape_character)
...
def _copy_sources_are_narrow(
    ...
    """Reject malformed, absolute, parent-traversing, and root COPY sources."""
    for directive, argument in instructions:
        if directive != "COPY":
            continue
...
def _from_reference(
    ...
    immutable = internal or not require_pinned_base or "@sha256:" in image
    ...
    for directive, argument in instructions:
        if directive == "FROM":
            immutable, alias = _from_reference(
                argument, aliases, stage_count, require_pinned_base
            )
            if not immutable:
                return False
            if alias:
...
def _container_file_passes(text: str, require_pinned_base: bool) -> bool:
    ...
    immutable = _container_references_are_immutable(
        instructions, require_pinned_base, dockerfile.escape_character
    )
