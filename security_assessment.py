    ...
    try:
        for path in _workflow_files(root):
            for line in path.read_text(encoding="utf-8").splitlines():
                code = line.split("#", 1)[0]
                keys = list(uses_key.finditer(code))
                matches = list(uses_value.finditer(code))
                unparsed += len(keys) - len(matches)
                references.extend(next(value for value in match.groups() if value) for match in matches)
...
def _docker_instructions(text: str) -> list[tuple[str, str]] | None:
    ...
    for raw_line in text.splitlines():
        ...
        if continued:
            continue
        match = re.fullmatch(r"([A-Za-z]+)\s+(.+)", current.strip())
        if match is None:
            return None
...
def _copy_sources(argument: str) -> list[str] | None:
    """Extract Docker COPY sources for shell and JSON instruction forms."""
...
def _container_file_passes(text: str, require_pinned_base: bool) -> bool:
    ...
    for directive, argument in instructions:
        if directive == "FROM":
            ...
            if sources is None:
                return False
            normalized = {source.replace(chr(92), "/").rstrip("/") for source in sources}
            if "." in normalized:
                return False
    pinned = bool(base_images) and all("@sha256:" in image for image in base_images)
    return final_user == "10001:10001" and (pinned or not require_pinned_base)
