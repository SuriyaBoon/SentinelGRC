...
class AgentKeyTests(unittest.TestCase):
    ...
    def test_explicit_empty_scopes_is_rejected(self):
        # scopes=() must not silently fall back to the posture-write default -
        # a caller explicitly asking for zero route access must get zero
        # route access, not posture:write by accident.
        with tempfile.TemporaryDirectory() as directory:
            registry = AgentKeyRegistry(str(Path(directory) / "keys.db"))
            for empty_scopes in ((), []):
    ...
    def test_omitted_scopes_still_default_to_posture_write(self):
        # The fix must not regress the documented default: omitting the
        # scopes argument entirely (scopes=None) still grants posture:write.
        with tempfile.TemporaryDirectory() as directory:
            registry = AgentKeyRegistry(str(Path(directory) / "keys.db"))
