...
SQLITE_LOCK_RETRY_SECONDS = 0.05


def is_retryable_sqlite_lock(error: sqlite3.OperationalError) -> bool:
    """Return whether an SQLite error is a primary or extended lock code."""
...
def enable_sqlite_wal(connection: sqlite3.Connection) -> None:
    """Require WAL mode, retrying only bounded transient lock failures."""
        ...
        try:
            ...
            if mode != "wal":
                raise sqlite3.OperationalError(
                    f"SQLite WAL activation failed; journal mode is {mode!r}"
                )
            return
...
class SQLiteStateStore:
    def __init__(
        ...
        *,
        ...
        with closing(self._connect()) as connection:
            connection.execute(f"PRAGMA busy_timeout={SQLITE_LOCK_TIMEOUT_SECONDS * 1000}")
    ...
    def _connect(self) -> sqlite3.Connection:
        connection = sqlite3.connect(database=self.path, timeout=SQLITE_LOCK_TIMEOUT_SECONDS, uri=False)
