...
class IngestionSecurityTests(unittest.TestCase):
    ...
    def test_naive_collected_at_is_rejected(self):
        ...
        with self.assertRaises(ValueError):
            validate_posture(invalid)
    def test_excessive_timestamp_precision_returns_structured_client_error(self):
        invalid = json.loads(self.body)
        invalid["collected_at"] = "2026-07-16T10:00:00.0000001Z"
        with self.assertRaisesRegex(
            ValueError, re.escape("collected_at must be an ISO-8601 timestamp.")
        ):
            validate_posture(invalid)
        with tempfile.TemporaryDirectory() as directory:
            status, response = self._dispatch(
    ...
    def test_false_commit_is_accepted_only_when_same_identity_is_committed(self):
        class ConcurrentCommitState(MemoryPayloadState):
            def __init__(self, committed_id_matches):
                super().__init__()
                self.committed_id_matches = committed_id_matches

            def commit_payload(self, payload_hash, evidence_id):
                self.pending.pop(payload_hash, None)
                self.evidence_ids[payload_hash] = (
                    evidence_id if self.committed_id_matches else "different-evidence-id"
                )
                return False

        with tempfile.TemporaryDirectory() as directory:
            handler = PostureHandler.__new__(PostureHandler)
            state = ConcurrentCommitState(True)
            handler.server = SimpleNamespace(
                state_store=state, publication_lock=threading.Lock()
            )
            responses = []
            handler._send_json = lambda status, response: responses.append((status, response))
            handler._persist_validated_body(
                self.body, {"record_type": "posture"}, Path(directory)
            )
            self.assertEqual(responses[0][0], HTTPStatus.ACCEPTED)

        with tempfile.TemporaryDirectory() as directory:
            handler = PostureHandler.__new__(PostureHandler)
            state = ConcurrentCommitState(False)
            handler.server = SimpleNamespace(
                state_store=state, publication_lock=threading.Lock()
            )
            handler._send_json = lambda status, response: None
    ...
    def test_post_bind_startup_failure_closes_the_bound_socket(self):
        from scripts.ingestion_api import IngestionServer

        captured = {}

        def fail_after_bind(server, *args):
            captured["address"] = server.server_address
            raise RuntimeError("startup failed")

        address = ("127.0.0.1", 0)
        registry = StaticKeyRegistry(b"unused", ())
        posture_dir = Path("posture")
        state_db = Path("state.db")
        runtime_root = Path(".")
        with mock.patch.object(IngestionServer, "_initialize_runtime", fail_after_bind):
            with self.assertRaisesRegex(RuntimeError, "startup failed"):
                IngestionServer(
                    address, registry, {}, posture_dir, {}, state_db, runtime_root,
                )
        with socket.socket() as probe:
            probe.bind(captured["address"])
