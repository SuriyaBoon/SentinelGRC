...
def assess(args: argparse.Namespace) -> int:
    ...
    with open("runtime/control-assessment.json", "w", encoding="utf-8") as output:
        json.dump(queue, output, indent=2)
...
def main() -> int:
    ...
    Path("runtime").mkdir(parents=True, exist_ok=True)
    with open("runtime/remediation-queue.json", "w", encoding="utf-8") as output:
