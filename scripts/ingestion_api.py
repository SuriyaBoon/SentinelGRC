...
DEFAULT_ASSET_CONTEXT_DIR = "runtime/portfolio-inbox/asset-context"
DEFAULT_REMEDIATION_TICKET_DIR = "runtime/portfolio-inbox/remediation-ticket"
...
class PortfolioRoute(NamedTuple):
    record_type: str
    identity_field: str
...
def _validate_identity_fields(payload: dict[str, Any]) -> None:
    if not is_canonical_text(payload["asset_id"], 128) or IDENTIFIER.fullmatch(payload["asset_id"]) is None:
...
def _validate_posture_context(payload: dict[str, Any]) -> None:
    ...
    if "criticality" in payload and (
        not isinstance(payload["criticality"], str)
        or payload["criticality"] not in CRITICALITIES
    ):
        ...
        with self.server.publication_lock:
            ...
            if existing_id:
                ...
                if not self.server.state_store.commit_payload(payload_hash, evidence_id):
                    committed_id = self.server.state_store.get_evidence_id(payload_hash)
                    if committed_id != evidence_id:
                        raise sqlite3.OperationalError(
                            "payload publication state was not committed"
                        )
        if duplicate_id is not None:
            self._send_json(HTTPStatus.ACCEPTED, {
                "status": "duplicate", "evidence_id": duplicate_id, **response_fields,
            })
            return
        self._send_json(HTTPStatus.ACCEPTED, {
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            required_scope = (
                ...
                else portfolio_route.required_scope
            )
            body = self._read_authenticated_body(required_scope)
            if portfolio_route is None:
                self._persist_posture(body)
            else:
                self._persist_portfolio_record(
                    body,
                    portfolio_route.record_type,
                    portfolio_route.identity_field,
                    portfolio_route.normalizer,
                )
...
class IngestionServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        try:
            self._initialize_runtime(
                key_registry, key_secrets, output_dir, portfolio_output_dirs,
                state_db, runtime_root,
            )
    ...
    def _initialize_runtime(
        self, key_registry: AgentKeyRegistry, key_secrets: dict[str, str],
        output_dir: Path, portfolio_output_dirs: dict[str, Path],
        state_db: Path, runtime_root: Path,
    ) -> None:
        """Initialize post-bind state before serve_forever accepts requests."""
        ...
        # but nothing can be in flight yet: the socket is bound, but
        # serve_forever has not begun accepting requests. Using the default
...
def _resolve_ingestion_directories(
    ...
    portfolio_output_dirs: dict[str, Path] = {}
    for route in dict.fromkeys(PORTFOLIO_ROUTES.values()):
        argument_name = f"{route.record_type}_output_dir"
        portfolio_output_dirs[route.record_type] = resolve_directory_under_root(
            getattr(args, argument_name, route.default_output_dir),
            runtime_root,
            purpose=f"{route.record_type.replace('_', '-')} ingestion directory",
            create=True,
        )
    resolved = [output_dir, *portfolio_output_dirs.values()]
    ...
    for index, directory in enumerate(resolved):
        for other in resolved[index + 1:]:
            if directory in other.parents or other in directory.parents:
                raise SystemExit(
                    "Posture and portfolio output directories must not be nested."
                )
...
def main() -> int:
    ...
    common.add_argument("--output-dir", default="evidence-inbox")
    common.add_argument(
        "--asset-context-output-dir", default=DEFAULT_ASSET_CONTEXT_DIR,
    )
    common.add_argument(
        "--remediation-ticket-output-dir", default=DEFAULT_REMEDIATION_TICKET_DIR,
    )
    common.add_argument("--state-db", default=DEFAULT_STATE_DB)
