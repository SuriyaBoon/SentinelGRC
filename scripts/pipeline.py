...
def _resolve_pipeline_paths(
    ...
    resolved_governance = (
        ...
        if governance_db else None
    )
    return _PipelinePaths(
        output_root,
        str(resolve_under_root(ledger_path, output_root, purpose="ledger path")),
        str(resolve_under_root(remediation_path, output_root, purpose="remediation path")),
        str(resolve_under_root(tickets_path, output_root, purpose="tickets path")),
        str(resolve_under_root(report_path, output_root, purpose="report path")),
        str(resolve_under_root(state_db, output_root, purpose="state database path")),
        resolved_audit,
        resolved_governance,
    )
...
def _validate_pipeline_inputs(
    ...
    return review, _input_hash(inputs)
...
def _persist_governance_findings(
    governance_db: str | None, remediation: dict[str, Any]
) -> None:
...
def run_pipeline(
    ...
    review, input_hash = _validate_pipeline_inputs(posture, controls, assets, access_review)
    ...
    try:
        ...
        _persist_governance_findings(paths.governance_db, remediation)
        ...
        report = _build_pipeline_report(
            run_id, created, asset, results, remediation, tickets, record, access_review,
        )
        ...
        return _accepted_pipeline_result(run_id, input_hash, record, paths, report)
