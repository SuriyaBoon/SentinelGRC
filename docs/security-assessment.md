...
| Dependency vulnerabilities | Pinned PyPA `pip-audit` result and report digest | Dependency owner | Upgrade, remove, or formally accept with expiry |
| Container boundary | Digest base, non-root runtime, explicit COPY set | Platform owner | Remediate before image publication |
...
The CI job runs a commit-pinned PyPA `pip-audit` action against the hash-locked
requirements file, then writes the fixed artifact:

```text
runtime/staging-assurance/security-assessment-evidence.json
```

The raw scanner output is not retained in the sanitized assessment. Only its
SHA-256 digest and the pinned tool reference are recorded. The checksum detects
accidental or post-collection modification; it is not a cryptographic
attestation of the runner or operator.
