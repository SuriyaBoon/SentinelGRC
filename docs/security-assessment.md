...
| Container boundary | Digest-pinned runtime base, effective final-stage non-root user in `Dockerfile`, `Dockerfile.assurance`, and `Dockerfile.qualification`, explicit COPY sources | Platform owner | Remediate before image publication |
...
production runtime image. The qualification overlay is built only for
`linux/amd64`; other architectures are outside this reviewed release path.
The pinned dependency-audit action scans both the
...
scanner failure so the reason for `NO_GO` is retained. On exit `2`, the
collector writes no artifact and `if-no-files-found: error` makes the upload
step fail. Treat this as a rejected assessment, not as missing evidence.
...
from the exact runtime layer and installs the hash-locked assessment dependency.
The runtime image runs the discovered production-compatible modules without
PyYAML. The qualification image runs only `test_security_assessment`,
`test_supply_chain_policy`, and `test_production_image_closure`. Only the runtime
and assurance images are saved and published. If assessment dependency setup
fails in CI, the standard
