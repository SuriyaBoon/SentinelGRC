...
class PortfolioContractTests(unittest.TestCase):
    ...
    def test_ticket_rejects_invalid_sla_context(self):
        cases = (
            ...
            ("due_at", "naive"),
            # Regression for the P1 finding: due_at earlier than created_at
            # previously normalized without error, persisting a ticket that
            # was already overdue before it existed.
    ...
    def test_ticket_allows_due_at_equal_to_created_at(self):
        # A zero-duration SLA window (due_at == created_at) is unusual but
        # not internally inconsistent - the P1 fix must reject only due_at
        # strictly earlier than created_at, not this boundary case.
        ticket = self.ticket()
    ...
    # Fields that are intentionally narrower in the normalized record than
    # in the source input: normalize_*_v1() always converts to UTC and
    # emits a trailing "Z" (NORMALIZED_RFC3339_PATTERN), while the source
    # input schema accepts any RFC3339 offset (RFC3339_PATTERN). Every other
    # shared property must be defined identically between the two schemas.
            ...
            for field_name, source_field in source["properties"].items():
                ...
                with self.subTest(schema=source_name, field=field_name):
                    if field_name in timestamp_fields:
                        ...
                        # every attribute other than the pattern must still match
                        self.assertEqual(
    ...
    def test_input_and_normalized_schemas_enforce_trust_boundary_via_jsonschema(self):
        for source_name, normalized_name, fixture_name, normalizer_name, derived, _ in self.TRUST_BOUNDARY_DEFINITIONS:
            fixture = getattr(self, fixture_name)
            normalizer = {
                "normalize_asset_context_v1": normalize_asset_context_v1,
                "normalize_remediation_ticket_v1": normalize_remediation_ticket_v1,
    ...
    def test_schema_format_assertion_rejects_impossible_calendar_values(self):
        # format: date-time is annotation-only in Draft 2020-12 unless the
        # validator opts into format assertion via FormatChecker(). Without
        # it, a syntactically well-formed but impossible timestamp like
        # "2026-99-99T99:99:99Z" satisfies RFC3339_PATTERN (which only
        # checks digit counts, not value ranges) and would silently pass
        # schema validation.
        ...
        # The runtime path every real request goes through is unaffected by
        # this schema/validator distinction: parse_rfc3339() calls
        # datetime.fromisoformat(), which raises on an impossible calendar
        # value regardless of the regex or which validator constructed it.
        payload = self.asset()
