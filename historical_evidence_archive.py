...
ARCHIVE_FILES = {"README.md", "manifest.json", "SHA256SUMS.txt"}
...
def verify_archive(root: str | Path) -> dict[str, Any]:
    ...
    manifest = load_manifest(archive_root / "manifest.json")
    expected = _read_expected_checksums(archive_root / "SHA256SUMS.txt")
    actual = {
        "README.md": hashlib.sha256((archive_root / "README.md").read_bytes()).hexdigest(),
