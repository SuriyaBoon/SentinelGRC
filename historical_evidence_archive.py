...
_SENSITIVE_VALUE_PATTERNS = (
    ...
    re.compile(r"\b[a-z0-9-]+\.(?:azure|windows)\.(?:com|net)\b", re.IGNORECASE),
...
def _read_archive_file(path: Path, label: str) -> bytes:
    try:
        return path.read_bytes()
    except OSError as error:
        raise ValueError(f"historical evidence {label} cannot be read") from error
    ...
    actual = {
        README_NAME: hashlib.sha256(
            canonical_text_bytes(
                _read_archive_file(archive_root / README_NAME, README_NAME),
                README_NAME,
            )
        ).hexdigest(),
