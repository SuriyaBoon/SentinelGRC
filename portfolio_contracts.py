...
def _timestamp(payload: dict[str, Any], name: str, label: str) -> str:
    ...
    parsed = parse_rfc3339(value, label, name)
...
def normalize_remediation_ticket_v1(payload: Any) -> dict[str, Any]:
    ...
    parsed_created_at = parse_rfc3339(created_at, label, "created_at")
    if parse_rfc3339(updated_at, label, "updated_at") < parsed_created_at:
        raise ValueError("remediation ticket updated_at cannot precede created_at")
    if parse_rfc3339(due_at, label, "due_at") < parsed_created_at:
