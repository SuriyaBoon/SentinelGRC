...
def _timestamp(
    ...
    parsed = parse_rfc3339(value, label, name).astimezone(timezone.utc)
...
def _evidence_refs(value: Any, label: str) -> list[str]:
    ...
    for item in value:
        if (
            not is_canonical_text(item, 512)
...
def normalize_asset_context_v1(payload: Any) -> dict[str, Any]:
    ...
    observed_at, _ = _timestamp(value, "observed_at", label)
...
def normalize_remediation_ticket_v1(payload: Any) -> dict[str, Any]:
    ...
    created_at, parsed_created_at = _timestamp(value, "created_at", label)
    updated_at, parsed_updated_at = _timestamp(value, "updated_at", label)
    due_at, parsed_due_at = _timestamp(value, "due_at", label)
    if parsed_updated_at < parsed_created_at:
        raise ValueError("remediation ticket updated_at cannot precede created_at")
    if parsed_due_at < parsed_created_at:
        raise ValueError("remediation ticket due_at cannot precede created_at")
    references = _evidence_refs(value.get("evidence_refs"), label)
