...
def _evidence_refs(value: Any, label: str) -> list[str]:
    ...
    for item in value:
        ...
        if parsed.scheme in {"https", "azblob", "sample"} and not parsed.netloc:
            raise ValueError(f"{label} evidence reference is incomplete")
...
def normalize_asset_context_v1(payload: Any) -> dict[str, Any]:
    ...
    if value.get("schema_version") != "asset_context.v1":
        raise ValueError("asset context schema_version must be asset_context.v1")
    source = _identifier(value, "source", label).lower()
    ...
    criticality = _text(value, "criticality", 16, label).lower()
    status = _text(value, "status", 16, label).lower()
    ...
    if hostname is not None and (
        not isinstance(hostname, str) or not hostname.strip() or len(hostname.strip()) > 255
    ):
        raise ValueError("asset context hostname is invalid")
    references = _evidence_refs(value.get("evidence_refs"), label)
    identity = "|".join(("asset_context.v1", source, source_asset_id, asset_id))
    return {
        "context_id": "ASSET-CTX-" + hashlib.sha256(identity.encode()).hexdigest()[:16].upper(),
        "schema_version": "asset_context.v1", "source": source,
...
def normalize_remediation_ticket_v1(payload: Any) -> dict[str, Any]:
    ...
    if value.get("schema_version") != "remediation_ticket.v1":
    ...
    asset_id = _identifier(value, "asset_id", label)
    owner = _identifier(value, "owner", label)
    status = _text(value, "status", 32, label).lower()
    priority = _text(value, "priority", 2, label).upper()
    ...
    references = _evidence_refs(value.get("evidence_refs"), label)
    identity = "|".join(("remediation_ticket.v1", source, source_ticket_id, finding_id))
    return {
        "ticket_context_id": "TICKET-CTX-" + hashlib.sha256(identity.encode()).hexdigest()[:16].upper(),
        "schema_version": "remediation_ticket.v1", "source": source,
