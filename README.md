...
| Connector contracts | Complete for the audited offline source profiles | The signed-event, LogWatcher, JML-Automation, and Mini-SOAR boundaries fail closed under repository tests and are bound to the revisions and explicit accepted profiles in [`docs/connector-contracts.md`](docs/connector-contracts.md). No connector is claimed production-connected, and this grants no live-gate credit. |
