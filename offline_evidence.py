...
def validate_evidence_document(value: Any) -> dict[str, Any]:
    ...
    if document["schema_version"] != EVIDENCE_SCHEMA or document["mode"] != "offline_no_azure":
    ...
    results = _expect_exact_keys(
        document["results"],
        {"alert_count", "first_ingestion", "replay", "offline_gates", "offline_decision"},
    ...
    for label in ("first_ingestion", "replay"):
        summary = _expect_exact_keys(
            results[label],
            {"events_read", "findings_created", "findings_reassessed", "ignored", "errors"},
    ...
    if (
        not isinstance(gates, dict)
        or not gates
        or any(not isinstance(name, str) or not isinstance(result, bool) for name, result in gates.items())
    ):
        raise ValueError("offline evidence gates are invalid")
    if results["offline_decision"] not in {"READY_FOR_MANUAL_AZURE_STAGING", "NO_GO"}:
