...
def _directory_candidates(directory: Path) -> list[Path]:
    """Return deterministic JSON candidates, or no candidates on scan failure."""
    try:
...
def _reconcile_candidate(
    ...
    """Commit one verified pending candidate and return its evidence identity."""
    try:
        record = _pending_candidate(state_store, path, grace_seconds, current)
    except OSError:
        # A later sweep may succeed; never classify or commit unreadable bytes.
        return None
    if record is None:
        return None
    evidence_id = path.stem
    if not state_store.commit_payload(str(record["payload_hash"]), evidence_id):
        return None
    return evidence_id
...
def reconcile_pending_publications(
    ...
    *,
    ...
    for directory in directories:
        for path in _directory_candidates(directory):
            evidence_id = _reconcile_candidate(
