...
def read_evidence_bytes(path: Path) -> bytes | None:
    ...
    try:
        ...
        # Callers distinguish transient storage faults from permanent content
        # or file-type failures; do not collapse an OSError into None.
        raise
    finally:
    ...
    for directory in directories:
        ...
        for path in candidates:
            try:
                ...
                # A later sweep may succeed; never commit or classify content
                # while its bytes are temporarily unavailable.
                continue
            if record is None:
                continue
            evidence_id = path.stem
