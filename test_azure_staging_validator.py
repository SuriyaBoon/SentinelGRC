...
class AzureStagingValidatorTests(unittest.TestCase):
    ...
    def test_unexpected_http_status_fails_closed_without_token_leak(self):
        provider = SingleIdentityTokenProvider(ANALYST_ID, "analyst-subject")
    ...
    def test_four_role_isolated_phases_complete_real_governance_lifecycle(self):
        analyst_token = fake_token("analyst-subject")
        approver_token = fake_token("approver-subject")
        actors = {
            analyst_token: ActorContext(
                "analyst-subject", "analyst", auth_method="oidc"
            ),
            approver_token: ActorContext(
                "approver-subject", "approver", auth_method="oidc"
            ),
        }
        with tempfile.TemporaryDirectory() as directory:
            ...
            phases = (
                ("analyst", "analyst_prepare", ANALYST_ID, "analyst-subject"),
                ("approver", "approver_approve", APPROVER_ID, "approver-subject"),
                ("analyst", "analyst_remediate", ANALYST_ID, "analyst-subject"),
                ("approver", "approver_close", APPROVER_ID, "approver-subject"),
            )
            observed = []
            ...
            self.assertEqual(
                ...
                {"analyst-subject", "approver-subject"},
            )
